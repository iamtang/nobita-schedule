...
module.exports = app => {
  ...
  scheduleDir.map(async path => {
    const { schedule = {}, task } = requireJS(path);
    const key = Path.basename(path, '.js');
    const { type = 'worker', cron } = schedule;
    _schedule[key] = task;
    app.schedule = app.context.schedule = _schedule;
    if(!process.pids){
      console.log(chalk.red('【Nobita】请使用nobita-scripts启动进程，否则定时任务将会失效！https://nobitajs.github.io/nobita/#/scripts'));
      return;
    }

    if (
      app.config.env != 'local' && type == 'worker' && process.pids[0] == process.pid ||
      app.config.env != 'local' && type == 'all' ||
